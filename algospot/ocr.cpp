...
// begin_word_prob[i] i번째 단어가 첫 단어로 나올 확률

double begin_word_prob[501];
...
double recognize(int segment, int previousMatch) {
  ...
  for (int thisMatch = 1; thisMatch <= wordNum; thisMatch++) {
    ...
    // P(R|Q)는 원문이 Q일 때 분류기가 R을 반환할 확률: 따라서 P(R|Q)=∏[M(Q[i],
    // R[i])] => 여기서 M(a, b)=단어 a를 b로 분류할 확률

    // P(Q)=begin_word_prob(Q[0])*∏[rightAfter(Q[i-1], Q[i])]

    // f(Q)=P(R|Q)*P(Q)=∏[missMatched(Q[i], R[i])*rightAfter(Q[i-1],
    // Q[i])]=∏g(Q[i])

    // g(thisMatch)=rightAfter(previousMatch, thisMatch)*missMatched(thisMatch,
    // R[segment])

    double candidate = rightAfter[previousMatch][thisMatch] +
                       missMatched[thisMatch][classified[segment]] +
...
void initialize() {
  for (int i = 0; i < composition; ++i) {
    for (int j = 0; j <= wordNum; ++j) cache[i][j] = 1.0;
  }
  // memset(cache, 1.0, sizeof(cache[0][0])*composition*(wordNum + 1));
}
...
int main(void) {
  ...
  for (int i = 1; i <= wordNum; i++) {
    cin >> begin_word_prob[i];
    begin_word_prob[i] = log(begin_word_prob[i]);  // log로 변환
  }
  ...
  for (int i = 0; i <= wordNum; i++)
    for (int j = 1; j <= wordNum; j++) {
      ...
      //그렇게 하면 P(Q)=∏(rightAfter(Q[i-1], Q[i])) =>
      // begin_word_prob(Q[0])*∏(rightAfter(Q[i-1], Q[i]))보다 간단해졌다

      if (i == 0)

        rightAfter[i][j] = begin_word_prob[j];
