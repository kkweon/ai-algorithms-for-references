...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    dim_row = (N + num_landmarks) * 2 # init + num of movement + num of landmark
    Omega = matrix()
    Omega.zero(dim_row, dim_row)
    Omega.value[0][0] = 1.0
    Omega.value[1][1] = 1.0
    ...
    for move_idx in xrange(len(data)):
        omega_idx = move_idx * 2 # current position

        measurements = data[move_idx][0] #[[id, dx_landmark, dy_landmark], [...], ...]
        motion = data[move_idx][1] #[dx, dy]

        # Measurement begins
        for landmark_measurement in xrange(len(measurements)):
            landmark_id, dx_to_landmark, dy_to_landmark = measurements[landmark_measurement]
            measurement = [dx_to_landmark, dy_to_landmark]
            #print "landmark_id = ", landmark_id # 0 --- 4
            omega_idx_for_landmark = 2 * (N + landmark_id)

            # Measurement begins
            # For example, L0 - X0 = DX or X0 - L0 = -DX
            for x_or_y in xrange(2):
                # x_or_y  == 0? x: y
                Omega.value[omega_idx + x_or_y][omega_idx + x_or_y] += 1.0 / measurement_noise
                Omega.value[omega_idx_for_landmark + x_or_y][omega_idx_for_landmark + x_or_y] += 1.0 / measurement_noise

                Omega.value[omega_idx + x_or_y][omega_idx_for_landmark + x_or_y] += -1.0 / measurement_noise
                Omega.value[omega_idx_for_landmark + x_or_y][omega_idx + x_or_y] += -1.0 / measurement_noise

                Xi.value[omega_idx + x_or_y][0] += -measurement[x_or_y] / measurement_noise
                Xi.value[omega_idx_for_landmark + x_or_y][0] += measurement[x_or_y] / measurement_noise

        # Motion begins
        # Self Position Adds One (For example, X0 - X1 = - DX or X1 - X0 = DX)
        for x_or_y in xrange(4):
            Omega.value[omega_idx + x_or_y][omega_idx + x_or_y] += 1.0 / motion_noise

        # Other Position Adds -1 (For example, X0 - X1 = - DX or X1 - X0 = DX)
        for x_or_y in xrange(2):
            Omega.value[omega_idx + x_or_y][omega_idx + x_or_y + 2] += -1.0 / motion_noise
            Omega.value[omega_idx + x_or_y + 2][omega_idx + x_or_y] += -1.0 / motion_noise

            Xi.value[omega_idx + x_or_y][0] += -motion[x_or_y] / motion_noise
            Xi.value[omega_idx + x_or_y + 2][0] += motion[x_or_y] / motion_noise 
