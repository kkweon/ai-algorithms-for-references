...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    #
    #
    dim = 2 * (1 + num_landmarks) # because we only count recent movements
    Omega = matrix()
    Omega.zero(dim, dim)
    Xi = matrix()
    Xi.zero(dim, 1)
    ...
    for move in xrange(len(data)):
        omega_idx = 0 # because only the latest move stays

        measurement = data[move][0] #landmark_id, landmark_dx, landmark_dy
        motion = data[move][1] #[dx, dy]

        Omega = Omega.expand(dim + 2, dim + 2, [0, 1] + range(4, dim + 2))
        Xi = Xi.expand(dim + 2, 1, [0, 1] + range(4, dim + 2), [0])

        for landmark_id, landmark_dx, landmark_dy in measurement:
            omega_idx_for_landmark = 2 * (2 + landmark_id)
            measure_sense = [landmark_dx, landmark_dy]
            ...
            for x_or_y in xrange(2):
                ...
                Xi.value[omega_idx + x_or_y][0] += -measure_sense[x_or_y] / measurement_noise
                Xi.value[omega_idx_for_landmark + x_or_y][0] += measure_sense[x_or_y] / measurement_noise

        for i in xrange(4):
            # x,y -> new_x, new_y 
            Omega.value[omega_idx + i][omega_idx + i] += 1.0 / motion_noise

        for i in xrange(2):
            Omega.value[omega_idx + i][omega_idx + i + 2] += -1.0 / motion_noise
            Omega.value[omega_idx + i + 2][omega_idx + i] += -1.0 / motion_noise

            Xi.value[omega_idx + i][0] += -motion[i] / motion_noise
            Xi.value[omega_idx + i + 2][0] += motion[i] / motion_noise
