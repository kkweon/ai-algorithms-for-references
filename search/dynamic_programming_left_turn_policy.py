...
def is_valid(grid, location):
    ...
    if row < 0 or col < 0 or row > max_row - 1 or col > max_col - 1 or grid[row][col] != 0:
        # out of grid
        return False
    return True
...
def optimum_policy2D(grid, init, goal, cost):
    ...
    [[' ', ' ', ' ', 'R', '#', 'R'],
     ...
     [' ', ' ', ' ', '#', ' ', ' ']]
    """
    policy2D = [[' ' for col in xrange(len(grid[0]))]
                for row in xrange(len(grid))]

    c_row, c_col, c_orientation = init[0], init[1], init[2]
    value = [
        [[999 for col in xrange(len(grid[0]))] for row in xrange(len(grid))] for d in xrange(len(forward))
    ]
    policy = [
        [[' ' for col in xrange(len(grid[0]))] for row in xrange(len(grid))] for d in xrange(len(forward))
    ]
    change = True
    while change:
        change = False
                    ...
                    if goal[0] == row and goal[1] == col:
                        ...
                        for m_idx in xrange(len(action)):
                            ...
                            if is_valid(grid, new_loc):
                                new_value = value[new_orientation][
                                    new_row][new_col]
